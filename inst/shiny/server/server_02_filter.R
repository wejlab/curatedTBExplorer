...
reactive({ #apparently need to be wrapped in reactive to work
  ...
  observeEvent(input$filter_tb_btn, {
    ...
    if(tb_status == "PTB"){ my_data(as.data.frame(colData(PTB)))
    }else { my_data(as.data.frame(colData(LTBI)))}
  ...
  observeEvent(input$filter_hiv_btn, {
    ...
    if(hiv_status == "Positive"){ my_data(as.data.frame(colData(HIV_Positive)))
    }else { my_data(as.data.frame(colData(HIV_Negative)))}
  })
  observeEvent(input$filter_diabetes_btn, {
    ...
    if(diabetes_status == "Positive"){ my_data(as.data.frame(colData(Diabetes_Positive)))
    }else { my_data(as.data.frame(colData(Diabetes_Negative)))}
    
  })
  
#})
output$filter_summary_table <- renderTable({ #outputs the table
  my_data()
}, rownames = TRUE)
